...
## 자유 로그
- 프론트, 백엔드, 데이터베이스를 연계해서 초보 웹개발을 해볼 수 있어서 좋았습니다.
- 작업 중간에 로컬서버를 키는 과정이 있는데, 어떤 작업을 할때 켜야 하는지, 계속 켜져 있는 건지(아마 컴퓨터를 끄면 꺼질 것 같은데) 아직 헷갈립니다.
- 토큰이 만료되면 github 저장소를 쓸 수 없는 건지 궁금합니다.
